                                  ...
                                  {order.payment_status === 'Awaiting Verification' && (() => {
                                    ...
                                    const itemsSummary = order.items && order.items.length > 0
                                      ? order.items.map((i: any) => {
                                          const name = i.watch?.name || i.product?.name || 'Item';
                                          const id = i.product_id || i.watch?.id || i.product?.id;
                                          return `• Product: ${name}\n  Link: ${origin}/watches/${id}`;
                                        }).join('\n\n')
                                      : '';
                                    const profileWaText = `Hi StylishTick, I want to complete payment for order #${order.order_number} worth ₹${order.total_amount.toLocaleString()}.${itemsSummary ? `\n\nOrder Details:\n${itemsSummary}` : ''}`;
                                    const profileWaUrl = `https://wa.me/919699986430?text=${encodeURIComponent(profileWaText)}`;
