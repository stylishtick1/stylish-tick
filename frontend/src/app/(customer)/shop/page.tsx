...
function ShopContent() {
  ...
  useEffect(() => {
    ...
    fetchWatches();
  }, [search, selectedBrand, selectedCategory, minPrice, maxPrice, inStockOnly, sortBy, page, initialFeatured]);
        ...
        <aside className="w-full lg:w-64 bg-card border border-border p-6 rounded-lg space-y-6 flex-shrink-0 hidden lg:block">
          ...
          <div className="flex items-center justify-between pt-2 border-t border-border/40">
            ...
            <button 
              ...
              className={`w-10 h-6 flex items-center rounded-full p-1 cursor-pointer transition-colors duration-300 ${inStockOnly ? 'bg-primary' : 'bg-muted border border-border'}`}
            >
              <div className={`bg-white dark:bg-black w-4 h-4 rounded-full shadow-md transform transition-transform duration-300 ${inStockOnly ? 'translate-x-4' : 'translate-x-0'}`} />
            </button>
          </div>
        </aside>
                  ...
                  return (
                    ...
                    >
                      <div className="aspect-square w-full relative bg-muted rounded overflow-hidden">
                        ...
                        />
                        {watch.stock === 0 && (
                          <span className="absolute inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center text-white text-[10px] font-bold uppercase tracking-widest">
                            Out Of Stock
                          </span>
                        )}
